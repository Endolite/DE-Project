...
\begin{document}
	\maketitle
	\noindent
	This is the dependent variable Stock Price of Google (\(S\)) and the independent variable Days (\(t\)). The data is taken from Yahoo Finance from the ticker GOOG. 
	...
	\(\mu\) is the amount that \(\Exp(S)\) changes, making it the coefficient of the linear regression of \(\Delta S/\Delta t\) against \(S\), so \(\mu \approx 0.233\). \\
	\(\sigma\) is simply the standard deviation of the stock price in the sample, so \(\s igma \approx 4.356\). \\
	The model then becomes
		\[\boxed{\dv{S_t}{t} = 0.233S_t + 4.356S_t\dv{W_t}{t}}\]
